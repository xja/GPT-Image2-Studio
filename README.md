...
| 模块 | 能力 |
| --- | --- |
| Studio | 提示词输入、参考图上传、比例选择、分辨率选择、思考等级选择、输出格式选择、实时生成状态 |
| 图片拆解 | 上传单张源图生成高信息密度拆解信息图，支持比例、分辨率、标注语言、自定义语言、两侧说明卡片和结果缩略图切换 |
| 套图模式 | 独立于提示词模式的电商营销套图生成，支持生成前计划预览、计划内单张提示词微调、独立参考图用途标签与智能识别、可选 Logo 上传与位置/底色控制、历史参考图重传后的手动绑定、4/6/8/10/12 张快捷数量、营销场景推荐角色组合、1577 个四级类目行业模板的逐级选择和搜索、场景内角色专属提示词、12 个图片角色自选、目标语言、尺寸规格单位换算、中文图片类型文件名、带时间前缀的套图文件夹、单张提示词微调后重生成和未完成项补齐 |
...
- 参考图编排新增输出语言选择：在参考图编排视图里可选择简体中文、English、日本語或한국어；智能分析生成的场景提示词和后续正式生成都会追加目标语言约束，要求图片内标题、标签、注释和短文案使用所选语言，同时保留专有名词、商品名、型号、数字和单位。
- 套图模式目标语言扩展到 Français、Deutsch、Español，适合直接生成法语、德语和西班牙语跨境电商短文案。
- 套图规划会把商品描述和卖点更严格地分配给相关图片角色，避免把整段商品描述重复塞进无关图片；钓饵、LED、钢珠、ABS、本体、桨叶、鱼钩、USB 充电线等细节会优先进入材质、卖点、包装或使用步骤等匹配角色。
- 套图参考图绑定更精确：包装参考图只进入包装清单角色，详情信任图、对比图、活动图和口碑问答图不再默认混入包装参考；SKU 补图或重生成会从历史套图清单回填对应 SKU 主体参考，避免修复时丢失单个 SKU 的绑定关系。
- 套图清单会保存可选 Logo 设置和每张 SKU 图的主体元数据，历史记录、复用、单张重生成和补齐未完成项能继续沿用同一套参考关系。
- 套图结果卡片在桌面端改为更紧凑的 6 列布局，缩小卡片间距、预览图和操作按钮高度；移动端仍保留更高的触控按钮。
- 新增图片拆解模式：在“创作”菜单中独立打开，上传 1 张源图后生成带编号部件标注的拆解信息图，可选择标注语言、比例、分辨率和是否启用两侧说明卡片。
- 套图模式新增可选 Logo：支持单独上传 1 张 Logo，选择九宫格放置位置，并指定透明底直接放置或非透明底先抠图；Logo 会作为品牌标识参考加入套图计划和正式生成。
- 套图模式的尺寸规格支持公制、英制、双单位和原文保留；输入 `cm`、`mm`、`in`、`ft`、`ml`、`fl oz`、`kg`、`lb` 等规格后，可在尺寸规格图提示词中自动换算展示。
- 套图模式默认目标语言调整为 English，尺寸单位默认使用公制和英制双单位，更适合跨境电商图直接交付。
- 套图规划强化卖点图、场景图、种草图和材质细节图的角色意图，使提示词更强调痛点解决、真实使用场景、真人或结果佐证，以及多窗口材质细节分析。
- 钓饵类套图提示词进一步强化参考图硬件保真：保留同一 SKU 的轮廓、节数、头部方向、尾形、涂装、腹钩挂点和尾钩硬件，鱼线只能连接参考图中真实存在的线环、拖眼或连接环，腹部/尾部三本钩不得被移动到背部、侧面、鱼嘴或手上。
- 套图图片文件名改用中文图片类型标题，例如 `01-主图-...`、`02-卖点图-...`，避免 `hero`、`benefit` 等内部角色名进入交付文件。
- 套图输出文件夹新增 `HHMM-` 时间前缀，例如 `0908-AeroPressClear-abc12345`，同一天多次生成同一商品时更容易按时间区分。
- 上游 Responses 图像生成遇到 `408`、`5xx` 或 Cloudflare `520-524` 等短暂 HTTP 错误时会自动做有限重试，最终仍保留紧凑的错误提示。
- 亮色主题下的浮层、选择器、提示词模板面板和画廊日期分组头统一改用主题化表面色，避免局部组件残留深色背景。
...
套图模式面向单个商品生成一组电商营销图。填写商品名称、商品描述和卖点后，可以选择 4 / 6 / 8 / 10 / 12 张套图数量、营销场景、目标语言、比例、分辨率和输出格式；行业模板放在参数区下方，默认只显示一个当前类目按钮，点开后在悬浮下拉层中逐级选择一级、二级、三级和四级类目，也可按三级/四级类目名或编码搜索。选中四级类目后，计划器会把类目路径和对应拍摄重点写入每张生图提示词。也可以上传只属于套图模式的参考图，并为每张参考图标注商品主体、包装清单、材质细节、使用场景或风格参考；可选 Logo 会单独上传，并可选择放置位置和透明底/抠图处理方式。智能识别如果拿到明确类目线索，会自动切换到匹配的四级类目模板。复用历史套图后，如果历史参考图需要重传，上传卡片会显示“绑定历史参考图”选择项，可把当前文件手动绑定到指定历史参考图并继承其用途与备注。正式生成前可先点“预览计划”检查每张图的角色、营销文案和提示词，并对单张计划做提示词微调后再生成。
...
| 思考等级 | `low`、`medium`、`high`、`xhigh` | 默认 `xhigh`；约 `30s+`、`90s+`、`150s+`、`210s+` |
| 图片质量 | `high` | 当前默认高质量 |
...
| 目标语言 | 简体中文、English、日本語、한국어、Français、Deutsch、Español | 控制图片内短营销文案语言，同时保留品牌名、型号、数字和单位 |
| 套图参考图 | 最多 6 张 | 只影响套图模式，不共享普通 Studio 的参考图状态 |
| 参考图用途 | 商品主体、包装清单、材质细节、使用场景、风格参考、其他 | 可手动标注，也可让“智能识别”生成建议后再手动应用；识别到明确四级类目时会主动切换行业模板 |
| Logo（可选） | 1 张图片；九宫格位置；透明底或非透明底先抠图 | 作为品牌标识参考加入每张套图提示词和正式生成，不占用普通套图参考图用途标签 |
...
| 参数 | 可选值 | 说明 |
| --- | --- | --- |
| 参考图 | 1-6 张 JPG / PNG / WebP | 用于分析多张参考图之间的主体、风格、材质和场景关系 |
| 比例 | `1:1`、`5:4`、`9:16`、`21:9`、`16:9`、`4:3`、`3:2`、`4:5`、`3:4`、`2:3` | 控制根据编排提示词正式生成图片时的构图比例 |
| 输出语言 | 简体中文、English、日本語、한국어 | 控制参考图编排结果和正式生成图片里的标题、标签、注释、短文案语言 |
| 分辨率 | `auto` 或当前比例支持的尺寸 | `auto` 会使用当前比例的默认尺寸 |
...
套图模式生成的图片不会进入普通画廊目录，而是保存到同级 `creation` 日期目录，并在 `Pictures/json/creation-sets/` 维护套图清单。套图文件夹带有生成时间前缀，便于同一天同一商品的多轮生成按时间区分。生成前计划预览只生成本地计划，不会写入图片文件；正式生成、后续单张提示词微调、重生成或补齐未完成项才会更新同一套图清单和同一 creation 文件夹。套图清单会记录参考图文件名、用途标签、智能识别备注和可选 Logo 设置：

```text
C:\Users\<你的用户名>\Pictures\YYYY-MM\MM-DD\YYYY-MM-DD-creation\HHMM-商品名-短ID\
```
