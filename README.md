...
| 套图模式 | 电商商品营销图、SKU 补图、品牌图批量生产 | 计划预览、套图生成队列、单图提示词微调、未生成图像补齐、12 张参考图、Logo 控制、四级类目模板 |
...
A/B 两组图片数量必须一致；C/D 组可为空，若启用也必须与 A 组数量一致。每个任务会携带 2 到 4 张参考图，并保留 A/B/C/D 的产品组标签。生成前可以在各组列表中调整顺序，选择上下或左右排列，以及正方形或矩形队形，再点击“开始快速溶图”。结果会显示在快速溶图预览区，并作为普通画廊资产保存，便于后续下载、查看和复用。
