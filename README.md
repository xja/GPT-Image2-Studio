...
| 快速溶图 | `/#quick-blend` | 上传 A 组和 B 组图片，按顺序配对生成溶图结果 |
...
快速溶图入口在 `/#quick-blend`，也可从创建菜单进入。它面向 A/B 两组图片的批量溶图：上传 A 组和 B 组后，系统会严格按顺序配对为 `A1+B1`、`A2+B2`、`A3+B3`，每一对提交一个独立生成任务。

两组图片数量必须一致；每个任务必须且只会携带两张参考图，一张 A 图和一张 B 图。生成前可以在两组列表中调整顺序，再点击“开始快速溶图”。结果会显示在快速溶图预览区，并作为普通画廊资产保存，便于后续下载、查看和复用。
