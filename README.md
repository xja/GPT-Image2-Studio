...
生成后的套图记录在顶部“资产”菜单里的“套图记录”查看。记录页支持搜索商品、场景、行业或语言，打开对应套图文件夹，复制已生成图片路径，并把历史套图显式复用回当前套图工作区继续补图或重生成。
...
套图记录页可以在每张图片卡片上直接查看大图、复制该图提示词、下载单张图片或复制该图相对路径；顶部可批量复制整套相对路径、完整本地文件路径，并可打开套图文件夹。
