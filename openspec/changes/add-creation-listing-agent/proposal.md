...
- Generate one listing package per distinct SKU subject; if no SKU subject exists, generate one main-product listing package.
...
- Require titles to begin with quantity, include size immediately after quantity when size is known, then use core search terms, long-tail terms, traffic terms, and descriptive terms without keyword stuffing.
...
- Frontend: Creation Mode settings and Creation record detail need Listing Agent controls, status, per-SKU output cards, copy actions, and export actions.
