...
4. prompts 必须包含 1 到 3 条结果；关系明确时给 1 条，存在明显不同场景或动作时给 2 到 3 条。
5. 每条 prompt 都要包含主体、参考图关系、场景、动作或姿态、镜头、光线、画面融合要求和必要限制。
6. 如果关系不明确，请在 risks 中说明待确认点，并让 prompt 保持保守，不要强行指定归属。
7. 不要识别或编造真人身份、姓名、年龄、品牌、地点或图片中不存在的敏感信息。`;
...
function getPromptAgentInstruction(mode) {
  return mode === REFERENCE_ORCHESTRATION_MODE
    ? REFERENCE_ORCHESTRATION_INSTRUCTION
    : PROMPT_AGENT_INSTRUCTION;
}
  ...
  return [
    {
      ...
      content: [
        ...
        ...normalizedImages.map((normalized) => ({
          type: "input_image",
