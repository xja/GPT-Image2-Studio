...
const CREATION_REFERENCE_ANALYSIS_INSTRUCTION = `你是一个套图参考图识别 agent。请分析用户上传的 1 到 9 张电商套图参考图，为每张图判断最适合影响套图生成的用途，并返回 JSON。
...
6. category_hint 写出最可能的四级类目名称；如果无法判断到四级类目，返回空字符串。
7. category_path 写出可判断的完整类目路径；如果无法判断，返回空字符串。
8. risks 写出不确定点，例如产品身份不清、包装缺失、参考图互相冲突或文字不可辨认。
9. 不要识别或编造真人身份、姓名、年龄、品牌、地点或图片中不存在的敏感信息。`;

const CREATION_REFERENCE_ANALYSIS_SKU_INSTRUCTION =
  "SKU grouping: also return sku_subjects. Each sku_subjects item must represent one distinct sellable product subject for an added SKU image. Group multiple photos of the same subject into one item. Do not create SKU subjects for accessories, packaging-only images, material-only closeups, scenes, or style references.";

const PORTRAIT_REFERENCE_ANALYSIS_INSTRUCTION = `You are a portrait person reference analysis agent. Analyze the uploaded person reference images and return one strict JSON object for a portrait photography workflow.
...
7. Do not infer real age, race, ethnicity, nationality, religion, health, disability, pregnancy, sexual orientation, real identity name, or other sensitive personal attributes.
8. If adult status is unclear, safety must say to use ordinary portrait or lifestyle styling and avoid sexualized, nude, lingerie, or adult-oriented direction.
9. Keep the summary editable and conservative, suitable for user confirmation before image generation.`;
...
export const CREATION_REFERENCE_ANALYSIS_JSON_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["summary", "category_hint", "category_path", "reference_roles", "sku_subjects", "risks"],
  properties: {
    summary: {
      type: "string",
    ...
    sku_subjects: {
      ...
      items: {
        ...
        properties: {
          ...
          note: {
            type: "string",
            description: "What must be preserved for this SKU image subject.",
          },
        },
      },
...
export function buildPromptAgentInput({ image, images, mode, targetLanguage, targetLanguageLabel } = {}) {
...
export function createPromptAgentRequestBody({
  ...
  const body = {
    ...
    input: buildPromptAgentInput({ image, images, mode, targetLanguage, targetLanguageLabel }),
    reasoning: {
  ...
  return Array.isArray(value)
    ? value
        .map((item, index) => {
          ...
          const id = String(item.id || item.subjectId || item.subject_id || `sku-${index + 1}`).trim();
