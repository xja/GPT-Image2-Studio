export function createConfigModelPickerController({
  ...
  if (typeof state.configModels.searchQuery !== "string") {
    state.configModels.searchQuery = "";
  }
  ...
  function getModelSearchQuery() {
    return String(state.configModels.searchQuery || "").trim();
  }

  function getVisibleModels() {
    const query = getModelSearchQuery().toLowerCase();
    if (!query) {
      return state.configModels.items;
    }
    return state.configModels.items.filter((modelId) => modelId.toLowerCase().includes(query));
  }
  ...
  function renderEmptySearchState(query) {
    ...
    refs.modelOptionsList.appendChild(empty);
  }

  function render() {
    const hasModels = state.configModels.items.length > 0;
    const pickerOpen = state.configModels.open && hasModels;
    const searchQuery = getModelSearchQuery();
    const visibleModels = getVisibleModels();

    refs.modelPickerToggle.hidden = !hasModels;
    refs.modelPickerToggle.disabled = !hasModels || state.configModels.loading;
    refs.modelPickerToggle.setAttribute("aria-expanded", String(pickerOpen));
    refs.modelOptionsList.hidden = !pickerOpen;
  ...
  function bindEvents() {
    ...
    refs.responsesModelInput.addEventListener("input", () => {
      state.configModels.searchQuery = refs.responsesModelInput.value;
      state.configModels.open = state.configModels.items.length > 0;
      render();
    });
