...
function formatCompactMonthYearPart(dateLike) {
  const date = normalizeDateValue(dateLike);
...
export function createTimestampedFilename(input, overrides = {}) {
  ...
  if (options.omitDatePrefix) {
    return `${formatCompactHourMinutePart(createdAt)}-${keyword}-${formatCompactMonthYearPart(createdAt)}-${idTail}.${format || "png"}`;
  }
...
export async function saveGeneratedAsset({
  ...
  const relativePath = relativeImagePathFor(filename, createdAt, relativeDir, normalizedMetadata);
  const imagePath = imagePathFor(filename, outputDir, createdAt, relativeDir, normalizedMetadata);
  const resolvedIndexPath = resolveIndexPath(outputDir, indexPath);
