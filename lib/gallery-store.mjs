...
async function findImageEntryByFilename(outputDir, filename) {
  const entries = await collectImageEntries(outputDir);
...
async function createGalleryItem({
  ...
  return {
    ...
    imageUrl: buildPublicAssetUrl(publicBasePath, normalizedRelativePath, createdAt),
    thumbnailUrl: buildPublicAssetUrl(publicBasePath, normalizedRelativePath, createdAt),
    createdAt,
...
export async function deleteGeneratedAsset({ outputDir, indexPath, filename }) {
  ...
  await withIndexLock(resolvedIndexPath, async () => {
    ...
    const discoveredEntry =
      Object.keys(metadata).length === 0 ? await findImageEntryByFilename(outputDir, filename) : null;
    const candidates = [
      resolveImagePath(outputDir, filename, metadata),
      join(outputDir, filename),
      discoveredEntry?.absolutePath,
    ];
...
export async function listGalleryItems({
  ...
  const discoveredEntries = await collectImageEntries(outputDir);
  for (const entry of discoveredEntries) {
    ...
    const metadata = (await readLegacyMetadata(entry.filename, outputDir)) || {};
    items.push(
      await createGalleryItem({
        outputDir,
        publicBasePath,
