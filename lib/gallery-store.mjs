...
const IMAGE_OUTPUT_DIRNAME = "image";
...
function defaultImageRelativeDir(createdAt) {
  const monthFolder = formatMonthFolder(createdAt);
  const dateFolder = formatDateFolder(createdAt);
  return `${monthFolder}/${dateFolder}/${datedAssetDirName(createdAt, IMAGE_OUTPUT_DIRNAME)}`;
}

function relativeImagePathFor(filename, createdAt, relativeDir) {
  const normalizedDir = normalizeRelativePath(relativeDir) || defaultImageRelativeDir(createdAt);
  return `${normalizedDir}/${filename}`;
}

function imagePathFor(filename, outputDir, createdAt, relativeDir) {
  return join(outputDir, ...relativeImagePathFor(filename, createdAt, relativeDir).split("/"));
}
...
export async function saveGeneratedAsset({
  ...
  const relativePath = relativeImagePathFor(filename, createdAt, relativeDir);
  const imagePath = imagePathFor(filename, outputDir, createdAt, relativeDir);
  const resolvedIndexPath = resolveIndexPath(outputDir, indexPath);
