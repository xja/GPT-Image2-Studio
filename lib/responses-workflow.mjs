...
export async function requestImageGeneration({
  ...
  while (true) {
    ...
    try {
      result = await consumeResponsesSse(response.body, {
        onEvent,
        statusHeartbeatMs,
      });
    } catch (error) {
      if (!isRetryableUpstreamTerminalError(error) || terminalRetryCount >= maxTransientHttpRetries) {
        throw error;
      }
      ...
      await emitEvent(onEvent, {
        type: "status",
        stage: "retrying_upstream",
        message: `${error.message}，正在重试 ${terminalRetryCount}/${maxTransientHttpRetries}`,
      });
      await wait(getTransientRetryDelayMs(transientHttpRetryDelayMs, terminalRetryCount));
      continue;
    }
