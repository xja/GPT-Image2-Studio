...
function expectedSize(source = {}) {
  return cleanString(source.dimensionSpecs).match(/\d+(?:\.\d+)?\s*(?:in|inch|inches|cm|mm|ft|oz|lb|ml|l)\b/i)?.[0] || "";
}
...
export function makeMockCreationListingDraft(source = {}) {
  ...
  return normalizeCreationListingDraft({
    ...
    status: "completed",
    title: [quantity, size, skuName, "Product Listing Draft"].filter(Boolean).join(" "),
    ...
    fiveBullets: [
      `${quantitySize} format keeps quantity and size visible.`,
    ...
    description: `${skuName} listing draft for US marketplace review.`,
    backendSearchTerms: `${skuName} product listing`,
    keywordBuckets: {
      exact: [skuName],
      longTail: [`${skuName} product listing`],
