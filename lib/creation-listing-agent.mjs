...
function expectedQuantity(source = {}) {
  const directCount = Number(source.skuBundleCount) || 0;
  const subjectCounts = Array.isArray(source.skuSubjects)
    ? source.skuSubjects
      .map((sku) => Number(sku.bundleCount || sku.subjectUnitCount || sku.subject_unit_count) || 0)
      .filter((count) => count > 1)
    : [];
  const count = (directCount > 1 ? directCount : subjectCounts[0]) || directCount || 1;
  return `${count} Pack`;
}
