...
export function syncActiveCreationQueueSet(creationState = {}, set, normalizeSet) {
  ...
  if (!activeQueueId || !set || typeof normalizeSet !== "function") {
    return;
  }

  const queueJob = getCreationQueueJobs(creationState).find((job) => job.id === activeQueueId);
  if (queueJob) {
    queueJob.set = normalizeSet(set);
  }
}
