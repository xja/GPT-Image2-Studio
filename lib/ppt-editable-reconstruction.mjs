...
export function normalizePptArtifactSlideModule(raw = {}, { slideNumber = 1 } = {}) {
  const source = raw && typeof raw === "object" ? raw : {};
  const normalizedSlideNumber = Math.max(1, Math.round(cleanNumber(source.slideNumber, slideNumber)));
  return {
    slideNumber: normalizedSlideNumber,
    moduleSource: validateArtifactModuleSource(source.moduleSource, normalizedSlideNumber),
...
export async function requestPptArtifactSlideModule({
  ...
  if (!apiKey) {
    throw new Error("API key is required for artifact slide reconstruction.");
  }
  if (!slide?.absolutePath) {
    throw new Error("Slide image path is required for artifact slide reconstruction.");
  }
  ...
  const response = await fetchImpl(`${normalizeBaseUrl(baseUrl)}/responses`, {
    ...
    body: JSON.stringify({
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: buildPptArtifactModulePrompt({
                title: outline?.title,
                slideNumber,
                slideTitle: slide.title || outlineSlide?.title,
              }),
      ...
      text: {
        format: {
          type: "json_schema",
          name: "ppt_artifact_slide_module",
          strict: true,
          schema: ARTIFACT_MODULE_SCHEMA,
        },
      },
    }),
  });
  ...
  if (!response.ok) {
    throw new Error(
      formatHttpErrorMessage({
        label: "Artifact slide reconstruction request failed",
        status: response.status,
        body: await response.text(),
      }),
    );
  ...
  const payload = await response.json();
  return normalizePptArtifactSlideModule(parseJsonText(extractResponseText(payload)), { slideNumber });
}
...
async function writeSlideModules({ slidesDir, manifestsDir, slides, manifests, artifactModules, deckTitle }) {
  await mkdir(slidesDir, { recursive: true });
  await mkdir(manifestsDir, { recursive: true });

  for (const slide of slides) {
    const slideNumber = Number(slide.slideNumber) || 1;
    const artifactModule = artifactModules?.find((entry) => Number(entry.slideNumber) === slideNumber);
    ...
    if (artifactModule?.moduleSource) {
      await writeFile(join(manifestsDir, `${fileStem}.artifact-module.json`), `${JSON.stringify({
        slideNumber,
        warnings: artifactModule.warnings || [],
        moduleSource: artifactModule.moduleSource,
      }, null, 2)}\n`, "utf8");
    }
    await writeFile(
      join(slidesDir, `${fileStem}.mjs`),
      artifactModule?.moduleSource || buildSlideModuleSource({ slide, manifest, deckTitle }),
      "utf8",
    );
  }
}
...
export async function buildEditablePptxReconstruction({
  ...
  for (const slide of sortedSlides) {
    ...
    try {
      const artifactModule = await requestPptArtifactSlideModule({
        baseUrl,
        apiKey,
        responsesModel,
        reasoningEffort,
      ...
      artifactModules.push(artifactModule);
      manifests.push(buildFallbackPptReconstructionManifest({ slide, outline }));
      warnings.push(...artifactModule.warnings.map((warning) => `Slide ${slide.slideNumber}: ${warning}`));
    } catch (artifactError) {
      const artifactMessage = artifactError instanceof Error ? artifactError.message : String(artifactError);
      warnings.push(`Slide ${slide.slideNumber} artifact module fallback: ${artifactMessage}`);
  ...
  if (result.code !== 0 && artifactModules.length > 0) {
    warnings.push(
      ["Artifact module build failed; retrying with element-level fallback.", result.stderr.trim(), result.stdout.trim()]
        .filter(Boolean)
        .join("\n"),
    );
    await writeSlideModules({
