...
function resolveDynamicComponents(dynamicPreset) {
...
export function buildSlideImagePrompts({ outline, theme = "", dynamicPreset = "smart" } = {}) {
  ...
  const dynamicComponents = resolveDynamicComponents(dynamicPreset);
  ...
  return slides.map((slide, index) => {
    ...
    return {
      ...
      prompt: [
        ...
        `视觉说明：${visualBrief}`,
        `视觉风格：${visualTheme}`,
        `高级动态组件：${dynamicComponents}。这是静态图片型幻灯片，请用视觉动势、分层构图、路径线、光带、脉冲、浮层或阶段组件表达动态感，不要生成真实 UI 控件或播放按钮。`,
...
export function buildSlideEditPrompt({ outline, slideNumber, theme = "", editInstruction = "", dynamicPreset = "smart" } = {}) {
  ...
  return [
    ...
    `高级动态组件：${resolveDynamicComponents(dynamicPreset)}。`,
