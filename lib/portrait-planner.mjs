...
export function buildPortraitPlan(input = {}) {
  ...
  const items = Array.from({ length: imageCount }, (_, index) => {
    ...
    const prompt = [
      "Create a professional portrait photography image from the supplied person reference image(s), optional action and pose reference image(s), and optional clothing, prop, and accessory reference image(s).",
      ...
      "Use clothing, prop, and accessory reference image(s) only for wardrobe, material, styling, prop, and accessory cues; do not treat them as additional people.",
