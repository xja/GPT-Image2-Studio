...
export const CREATION_ITEM_ROLES = [
  ...
  },
  {
    ...
    brief: "photorealistic real-use recommendation image that shows a real user or credible use result with the product",
  },
  {
    role: "package",
    title: "包装清单图",
...
const CREATION_ROLE_INTENT_INSTRUCTIONS = {
  ...
  "social-proof":
    "Role intent: make this a photoreal real-use recommendation image, not a product-only graphic. Show an actual user or user-result proof with the product. For fishing lures or bait, show one coherent catch proof: an angler holding the caught fish, the same lure is attached to the fishing line through the exact line-tie, tow eye, or split ring that already exists on the reference lure; if the reference lure has a front/nose tow eye ahead of the diving lip, attach the line there, and show an original belly or tail treble hook point visibly set in the fish mouth, lip, or jaw. Keep the supplied reference lure's body, mouth, segments, tail, hooks, and hardware unchanged. Preserve the same SKU: same silhouette, segment count, head direction, tail shape, paint pattern, belly hook hangers, and tail hook hardware. Keep every belly treble hanging below the lure body at its original underside hanger position; do not move any belly hook, split ring, or hook hanger onto the back, spine, side, fish mouth, or angler hand, and the lure body must stay outside or beside the fish mouth while an original underside treble point hooks the mouth. The fish, lure, hook, line, and hand or rod must form one believable continuous tension chain. Do not show a disconnected lure, loose decorative hooks, fishing line floating past an empty ring, or a caught fish merely posed beside the lure; do not invent a top/back ring, dorsal ring, mouth ring, standalone hook, or new front attachment hardware. For ladders, show a real person safely standing on or using the ladder.",
  "usage-steps":
