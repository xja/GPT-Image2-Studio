...
const CREATION_ROLE_INTENT_INSTRUCTIONS = {
  ...
  "social-proof":
    "Role intent: make this a photoreal real-use recommendation image, not a product-only graphic. Show an actual user or user-result proof with the product. For fishing lures or bait, show one coherent catch proof: an angler holding the caught fish, the same lure is attached to the fishing line through the real top/back line-tie ring or split ring visible on the reference lure, and an existing lower treble hook point visibly set in the fish mouth, lip, or jaw. Keep the supplied reference lure's body, mouth, segments, tail, hooks, and hardware unchanged. The fish, lure, hook, line, and hand or rod must form one believable continuous tension chain. Do not show a disconnected lure, loose decorative hooks, fishing line floating past an empty ring, or a caught fish merely posed beside the lure; do not add a mouth hook, extra mouth ring, or new front attachment hardware. For ladders, show a real person safely standing on or using the ladder.",
  "usage-steps":
    "Role intent: teach the practical setup or operation with a clean step sequence while preserving the real product. Preserve the supplied reference product as the unchanged subject; use callout arrows, labels, hands, line, water, or small step panels around it, but do not redesign the lure body, paint pattern, segments, tail, hooks, lip, blade, or hardware. For fishing lures or bait, attach the fishing line through the actual visible top/back line-tie ring or tow eye on the reference lure, using the same physical attachment point consistently in the main image and step panels; do not tie the line to the body, eye, hook hanger, tail, mouth, propeller, diving lip, blade, or an invented ring, and do not add a hook or extra ring at the lure mouth.",
  "material-closeup":
