...
export const CREATION_INDUSTRY_TEMPLATE_OPTIONS = [
  {
    ...
    label: "通用电商",
    promptInstruction:
      "General ecommerce industry template: keep the set broadly useful for product listings, detail pages, and campaign assets.",
  },
  {
    value: "apparel",
    label: "服饰鞋包",
    promptInstruction:
      "Apparel and accessories industry template: emphasize fit, fabric texture, scale, styling, and accurate product proportions without impossible body shapes.",
  },
  {
    value: "beauty",
    label: "美妆个护",
    promptInstruction:
      "Beauty and personal care industry template: emphasize texture, swatches, skincare use, packaging, and benefit hierarchy without medical claims.",
  },
  {
    value: "food",
    label: "食品饮料",
    promptInstruction:
      "Food and beverage industry template: emphasize freshness, serving context, appetizing texture, packaging clarity, and ingredient cues without unsafe health claims.",
  },
  {
    value: "electronics",
    label: "3C 数码",
    promptInstruction:
      "Consumer electronics industry template: emphasize ports, dimensions, screen or device details, specification clarity, and comparison proof without fake UI or logos.",
  },
  {
    value: "home",
    label: "家居生活",
    promptInstruction:
      "Home and living industry template: emphasize room scale, material finish, installation or use context, and everyday lifestyle value without clutter.",
  },
];
...
export function normalizeCreationIndustryTemplate(value) {
  const normalized = cleanString(value);
  return (
    CREATION_INDUSTRY_TEMPLATE_OPTIONS.find((option) => option.value === normalized) ||
    CREATION_INDUSTRY_TEMPLATE_OPTIONS[0]
  );
}
...
export function getCreationIndustryRolePreset(value) {
  const industryTemplate = normalizeCreationIndustryTemplate(value);
  return normalizeCreationSelectedRoles(CREATION_INDUSTRY_ROLE_PRESETS[industryTemplate.value] || []);
}
