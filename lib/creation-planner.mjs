...
export function buildCreationPlan(input = {}) {
  ...
  const dimensionSpecLines = normalizeDimensionSpecs(dimensionSpecs);
    ...
    items: plannedRoles.map((role, index) => ({
      ...
      prompt: [
        ...
        role.role === "dimensions" && dimensionSpecSummary
          ? `Dimension specifications for this size chart only: ${dimensionSpecSummary}. Use these exact specifications only in the dimensions/specification image; other images may show broad size comparison, but do not print or reveal these exact values.`
          : "",
