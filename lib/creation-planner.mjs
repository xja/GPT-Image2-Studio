...
const DEFAULT_CREATION_IMAGE_COUNT = 10;
...
export const CREATION_ITEM_ROLES = [
  ...
  {
    role: "certification-proof",
    title: "资质背书图",
    filenameToken: "proof",
    brief: "certification and trust proof image that visualizes only explicitly provided certificates, test proof, safety marks, warranty terms, or quality checks",
  },
];
...
export const CREATION_SCENARIO_ROLE_INSTRUCTIONS = {
  ...
  "brand-story": {
    ...
    "certification-proof":
      "Role focus: present explicit trust proof, test evidence, quality checks, warranty terms, or certification marks only when they are supplied by the product inputs or references.",
    package:
...
const CREATION_ROLE_INTENT_INSTRUCTIONS = {
  ...
  "certification-proof":
    "Role intent: build a trust-proof module using only supplied evidence. If certificates, testing, safety marks, warranty terms, compliance labels, durability tests, or quality-control facts are provided, render them as clean proof cards around the product. If no explicit proof is provided, show a generic quality-check workflow without fake seals, fake certification names, warranty promises, medical claims, safety ratings, or unsupported badges.",
};
...
const CREATION_ROLE_RENDERING_CONSTRAINTS = {
  ...
  "certification-proof":
    "For this trust-proof image, do not invent certification marks, warranty promises, medical claims, test lab names, safety ratings, or compliance logos; use only proof explicitly present in the product input or references.",
};
...
const CREATION_ROLE_CONTENT_CATEGORIES = {
  ...
  "certification-proof": ["trust", "visible-copy", "material"],
};
...
function normalizeCreationReferenceAnalysisEntry(entry, index, filenames) {
  ...
  const role = normalizeCreationReferenceRole(
    shouldUseDimensionRole ? "dimensions" : shouldUseUsageRole ? "usage" : explicitRole?.value || inferredRole,
  );
