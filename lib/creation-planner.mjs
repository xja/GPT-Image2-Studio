...
export function buildCreationPlan(input = {}) {
  ...
  const plannedRoles = selectedRoles.length > 0 ? selectedRoles : CREATION_ITEM_ROLES.slice(0, imageCount);
  ...
  const productLine = productName || productDescription || sellingPoints[0];
  const descriptionLine = productDescription || "用户未提供详细描述";
  const sellingPointLine = sellingPoints.length > 0 ? sellingPoints.join(" / ") : "围绕商品核心价值提炼短卖点";
